    ...
    <div class="farm">
      <div class="farm-title">
        ...
        <div style="display: flex; gap: 8px;">
          <button class="ghost small" onclick="editFarm('${f.id}')">✏️ Editar</button>
          <button class="ghost small" onclick="deleteFarm('${f.id}')">🗑️ Eliminar</button>
        </div>
      </div>
      <div class="farm-meta">
        <span>📍 ${f.location}</span>
        <span>📏 ${f.size} ha</span>
        <span>🐄 ${f.animals} animales</span>
      </div>
      <div class="farm-meta">
        <span>🌱 ${f.soil}</span>
        <span>🏷️ ${f.license}</span>
        <span>🔧 ${f.management}</span>
        <span>🌾 ${f.feed}</span>
      </div>
    </div>
  `).join('');
...
animalForm?.addEventListener('submit', (e) => {
    e.preventDefault();
    if (!currentUser) {
        alert('Inicia sesión para registrar animales.');
        return;
    }
    ...
    animalsList.innerHTML = filtered.map(a => {
      ...
      <div class="farm">
        <div class="farm-title">
          ...
          <button class="ghost small" onclick="deleteAnimal('${a.id}')">🗑️ Eliminar</button>
        </div>
        <div class="farm-meta">
          <span>🏷️ ${a.breed}</span>
          <span>🌾 ${farmName}</span>
...
eventForm?.addEventListener('submit', (e) => {
    e.preventDefault();
    if (!currentUser) {
        alert('Inicia sesión para registrar eventos.');
        return;
    }
...
function renderEvents() {
    ...
    eventsList.innerHTML = filtered.map(e => {
        ...
        const typeColors = {
            'Vacunación': { bg: '#dbeafe', color: '#1e40af' },
      ...
      <div class="farm">
        <div class="farm-title">
          ...
          <button class="ghost small" onclick="deleteEvent('${e.id}')">🗑️ Eliminar</button>
        </div>
        <div class="farm-meta">
          <span>🐄 ${animalName}</span>
          <span>📅 ${formatDate(e.date)}</span>
          ${e.cost > 0 ? `<span>💰 ${e.cost.toFixed(2)}€</span>` : ''}
        </div>
        ${e.nextDate ? `<p class="status">📌 Próxima fecha: ${formatDate(e.nextDate)}</p>` : ''}
      </div>
    `;
    }).join('');
}
...
function renderFCRHistory() {
    ...
    if (sorted.length === 0) {
        fcrHistory.innerHTML = '<p class="status">No hay cálculos guardados</p>';
        return;
    }
    ...
    fcrHistory.innerHTML = sorted.map(h => {
      ...
      <div class="farm">
        <div class="farm-title">
          ...
          <button class="ghost small" onclick="deleteFCR('${h.id}')">🗑️ Eliminar</button>
        </div>
        <div class="farm-meta">
          <span>🐄 ${animalName}</span>
          <span>📅 ${formatDate(h.date)}</span>
          <span>⚖️ ${h.weightInit}kg → ${h.weightFinal}kg</span>
          <span>📈 ${h.gain.toFixed(1)}kg ganancia</span>
        </div>
        <div class="farm-meta">
          <span>🌾 ${h.feed}kg alimento</span>
          <span>📊 ${h.adg.toFixed(3)}kg/día</span>
          <span>⏱️ ${h.days} días</span>
        </div>
      </div>
    `;
    }).join('');
...
function exportToCSV(type) {
    ...
    if (type === 'inventory') {
        ...
        const events = getEvents();
        const animals = getAnimals();
        headers = ['Tipo', 'Animal', 'Fecha', 'Descripción', 'Costo (€)', 'Próxima Fecha'];
        data = events.map(e => {
            const animal = animals.find(a => a.id === e.animal);
            return [
                e.type,
        ...
        const history = getFCRHistory();
        const animals = getAnimals();
        headers = ['Animal', 'Fecha', 'Peso Inicial', 'Peso Final', 'Ganancia', 'Alimento', 'Días', 'FCR', 'GDP', 'Eficiencia'];
        data = history.map(h => {
            const animal = animals.find(a => a.id === h.animalId);
            return [
        ...
        const fincas = getFincas();
        headers = ['Nombre', 'Ubicación', 'Tamaño (ha)', 'Suelo', 'Licencia', 'Animales', 'Manejo', 'Alimentación'];
        data = fincas.map(f => [
            f.name,
            f.location,
            f.size,
...
function updateAlerts() {
    ...
    if (upcoming.length > 0) {
        alerts.push(`<li><span><span class="dot amber"></span> ${upcoming.length} evento(s) próximo(s)</span><span class="pill">⚠️</span></li>`);
    }
...
function populateSelects() {
    ...
    animalSelects.forEach(select => {
        ...
        select.innerHTML = isFilter ? '<option value="">Todos los animales</option>' : isFCR ? '<option value="">Cálculo general</option>' : '<option value="">Selecciona animal</option>';
        animals.forEach(a => {
            const option = document.createElement('option');
            option.value = a.id;
